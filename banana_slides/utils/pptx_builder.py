...
class PPTXBuilder:
    ...
    FONT_PATH = os.path.join(
        os.path.dirname(__file__), "..", "fonts", "NotoSansSC-Regular.ttf"
    )
    ...
    def __init__(
        self, slide_width_inches: float = None, slide_height_inches: float = None
    ):
        """
    ...
    def setup_presentation_size(
        self, width_pixels: int, height_pixels: int, dpi: int = None
    ):
        """
    ...
    def calculate_font_size(
        ...
        if usable_width_pt <= 0 or usable_height_pt <= 0:
            logger.warning(
                f"Bbox too small for text: {width_px}x{height_px}px, text: '{text[:30]}...'"
            )
        ...
        for font_size in range(
            int(self.MAX_FONT_SIZE), int(self.MIN_FONT_SIZE) - 1, -1
        ):
        ...
        if best_size == self.MIN_FONT_SIZE and text_length > 3:
            logger.warning(
                f"Text may overflow: '{text[:50]}...' in bbox {width_px}x{height_px}px"
            )
    ...
    def add_text_element(
        ...
        def normalize_bullet_chars(text: str) -> str:
            """Normalize bullet point characters for consistent rendering.

            Replaces middle dot (·) with bullet point (•) when it appears at the start of text.
            """
            text = text.replace("·", "•", 1) if text.lstrip().startswith("·") else text
            return text

        actual_text = normalize_bullet_chars(actual_text)
        ...
        if (
            text_style
            and hasattr(text_style, "text_alignment")
            and text_style.text_alignment
        ):
        ...
        if has_colored_segments:
            ...
            for seg in text_style.colored_segments:
                ...
                run.text = normalize_bullet_chars(seg.text)
            ...
            if (
                text_style
                and hasattr(text_style, "font_color_rgb")
                and text_style.font_color_rgb
            ):
                r, g, b = text_style.font_color_rgb
                paragraph.font.color.rgb = RGBColor(r, g, b)

            style_info = (
                f" | color={text_style.font_color_rgb if text_style else 'default'}"
            )
        ...
        bbox_height = bbox[3] - bbox[1]
        logger.debug(
            f"Text: '{actual_text[:35]}' | box: {bbox_width}x{bbox_height}px | font: {font_size:.1f}pt | chars: {len(actual_text)}{style_info}"
        )

    def add_image_element(
        self, slide, image_path: str, bbox: List[int], dpi: int = None
    ):
        """
    ...
    def save(self, output_path: str):
        ...
        if (
            str(output_dir) != "."
        ):  # Only create directory if it's not current directory
