...
from .ai_providers.image.volcengine_inpainting_provider import (
    VolcengineInpaintingProvider,
)

# from .ai_providers.image.gemini_inpainting_provider import GeminiInpaintingProvider
    ...
    def __init__(self, provider=None, provider_type: str = "volcengine"):
        ...
        if provider is None:
            ...
            if provider_type == "gemini":
                ...
                raise NotImplementedError("Gemini Inpainting Provider is not available")
            else:
    ...
    def remove_regions_by_bboxes(
        ...
        try:
            ...
            mask = create_mask_from_image_and_bboxes(
                image, bboxes, expand_pixels=expand_pixels
            )
    ...
    def regenerate_background(
        ...
        try:
            ...
            mask = create_inverse_mask_from_bboxes(
                image.size, foreground_bboxes, expand_pixels=expand_pixels
            )

            logger.info(f"反向掩码已生成，尺寸: {mask.size}")

            # 调用 inpainting 服务（已内置重试逻辑）
            result = self.provider.inpaint_image(original_image=image, mask_image=mask)
...
def get_inpainting_service(provider_type: str = None) -> InpaintingService:
    ...
    if provider_type is None:
        ...
        provider_type = getattr(
            config, "INPAINTING_PROVIDER", "gemini"
        )  # 默认使用 gemini
...
def remove_regions(
    image: Image.Image, bboxes: List[Union[Tuple[int, int, int, int], dict]], **kwargs
) -> Optional[Image.Image]:
    """
    便捷函数：消除图像中的指定区域
