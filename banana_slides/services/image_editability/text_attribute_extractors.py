...
from ..prompts import get_text_attribute_extraction_prompt
...
class TextStyleResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # 转换 colored_segments
        result["colored_segments"] = [
            seg.to_dict() if isinstance(seg, ColoredSegment) else seg
            for seg in self.colored_segments
        ]
        return result
...
class TextAttributeExtractor(ABC):
    ...
    def extract_batch(
        self, items: List[Tuple[Union[str, Image.Image], Optional[str]]], **kwargs
...
class CaptionModelTextAttributeExtractor(TextAttributeExtractor):
    ...
    def extract(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.error(
                f"CaptionModelTextAttributeExtractor提取失败: {e}", exc_info=True
            )
            return TextStyleResult(confidence=0.0, metadata={"error": str(e)})

    def _call_vision_model(
        self, image: Image.Image, prompt: str, thinking_budget: int
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            result = self.ai_service.generate_json_with_image(
                prompt=prompt, image_path=tmp_path, thinking_budget=thinking_budget
            )
    ...
    def extract_batch_with_full_image(
        ...
        **kwargs,
        ...
        import json
        import tempfile
        from banana_slides.services.prompts import get_batch_text_attribute_extraction_prompt
        ...
        try:
            ...
            text_elements_json = json.dumps(
                elements_for_prompt, ensure_ascii=False, indent=2
            )
    ...
    def _parse_batch_result(
        self, result_list: List[Dict[str, Any]], original_elements: List[Dict[str, Any]]
    ) -> Dict[str, TextStyleResult]:
        """
        解析批量提取的 AI 返回结果
        ...
        for item in result_list:
            try:
                ...
                logger.warning(
                    f"解析元素 {item.get('element_id', 'unknown')} 的样式失败: {e}"
                )
                continue

        logger.info(
            f"批量解析完成: 成功 {len(results)}/{len(original_elements)} 个元素"
        )
        return results
...
class TextAttributeExtractorRegistry:
    ...
    def register(
        self, element_type: str, extractor: TextAttributeExtractor
        ...
        logger.debug(
            f"注册文字属性提取器: {element_type} -> {extractor.__class__.__name__}"
        )
        return self

    def register_types(
        self, element_types: List[str], extractor: TextAttributeExtractor
    ) -> "TextAttributeExtractorRegistry":
        """
        批量注册多个元素类型到同一个提取器
    ...
    def register_default(
        self, extractor: TextAttributeExtractor
    ) -> "TextAttributeExtractorRegistry":
        """
    ...
    def get_extractor(
        self, element_type: Optional[str]
    ) -> Optional[TextAttributeExtractor]:
        """
        根据元素类型获取对应的提取器
    ...
    @classmethod
    def create_default(
        cls, caption_extractor: Optional[TextAttributeExtractor] = None
    ) -> "TextAttributeExtractorRegistry":
        """
        创建默认配置的注册表
        ...
        logger.info(
            f"创建默认TextAttributeExtractorRegistry: "
