...
class ExtractorFactory:
    ...
    def create_extractor_registry(
        ...
        try:
            ...
            if baidu_accurate_provider:
                baidu_accurate_ocr_extractor = BaiduAccurateOCRElementExtractor(
                    baidu_accurate_provider
                )
    ...
    def create_hybrid_extractor_registry(
        ...
        if baidu_table_ocr_provider is None:
            ...
            baidu_table_ocr_extractor = BaiduOCRElementExtractor(
        ...
        if baidu_table_ocr_extractor:
            registry.register_types(
                list(ExtractorRegistry.TABLE_TYPES), baidu_table_ocr_extractor
            )
...
class InpaintProviderFactory:
    ...
    def create_inpaint_registry(
        ...
        if generative_provider:
            # 图片和图表使用生成式重绘
            registry.register_types(
                ["image", "figure", "chart", "diagram"], generative_provider
            )

        logger.info(
            f"创建InpaintProviderRegistry: 默认={default_provider_type}, "
    ...
    def create_baidu_inpaint_provider() -> Optional[BaiduInpaintProvider]:
        raise NotImplementedError("Baidu Inpaint Provider is not available")
    ...
    def create_hybrid_inpaint_provider(
        ...
        raise NotImplementedError("Hybrid Inpaint Provider is not available")
...
class ServiceConfig:
    ...
    def from_defaults(
        ...
        extractor_method: Optional[
            str
        ] = None,  # 'mineru' 或 'hybrid'，优先于 use_hybrid_extractor
        inpaint_method: Optional[
            str
        ] = None,  # 'generative', 'baidu', 'hybrid'，优先于 use_hybrid_inpaint
        **kwargs,
    ) -> "ServiceConfig":
        """
        从默认参数创建配置
        ...
        if extractor_method is not None:
            ...
            logger.info(
                f"extractor_method={extractor_method} -> use_hybrid_extractor={use_hybrid_extractor}"
            )
        ...
        if has_app_context() and current_app:
            ...
            if mineru_api_base is None:
                mineru_api_base = current_app.config.get(
        ...
        parser_service = FileParserService(
            mineru_token=mineru_token, mineru_api_base=mineru_api_base
        )
...
class TextAttributeExtractorFactory:
    ...
    def create_caption_model_extractor(
        ai_service: Optional[Any] = None, prompt_template: Optional[str] = None
    ) -> TextAttributeExtractor:
        """
    ...
    def create_text_attribute_registry(
        ...
        # 注册文本类型
        registry.register_types(
            ["text", "title", "paragraph", "heading", "table_cell"], caption_extractor
        )
